...
.date {
    display: inline-block; /* 요소를 인라인 블록으로 표시하여 텍스트 주위에 박스를 넣습니다. */
    padding: 2px;
    border-radius: 2px;
}

.date.complete {
    color: #C8386B; /* 원하는 배경색을 지정합니다. */
...
function fn_mySearch()
{
	...
	document.myForm.action = "/user/mypageMyQ";
                                ...
                                <div class="css-134c4us ekeuxnk0">
                                    ...
                                    <a class="css-1p9dybm e1q26atk3" href="/user/mypageScrap">
                                        ...
                                        <div class="css-2rwq1u e1q26atk0">
                                            <fmt:formatNumber type="number" maxFractionDigits="3" value="${userCnt.scrapCnt}" />
                                        </div>
                                    </a>
...
<c:if test="${not empty list}">
          ...
          <div class="card">
			  ...
			  <div class="content-box">
			  	<div class="num" style="color:#282828;">카테고리</div>
			  	<div class="num" style="color:#282828;">글번호</div>
			  	<div class="title" style="text-align: center; style=color:#282828;">제목</div>
			  	<div class="date" style="color:#282828;">작성일</div>
			  	<div class="num" style="color:#282828;">답변여부</div>
			  </div>
			  
			  
<c:if test="${!empty list}">	
    <c:forEach var="csMy" items="${list}" varStatus="status">	
        <c:choose>
            <c:when test="${csMy.questionCategory eq 1}">
                <c:set var="categoryName" value="배송" />
            </c:when>
            <c:when test="${csMy.questionCategory eq 2}">
                <c:set var="categoryName" value="교환/취소" />
            </c:when>
            <c:when test="${csMy.questionCategory eq 3}">
                <c:set var="categoryName" value="주문/결제" />
            </c:when>
            <c:when test="${csMy.questionCategory eq 4}">
                <c:set var="categoryName" value="회원정보" />
            </c:when>
            <c:otherwise>
        ...
        <div class="content-box">
            ...
            <div class="num">${csMy.questionIdk}</div>
            <div class="title"><a href="/cs/inquirylistDetail?questionIdk=${csMy.questionIdk}">${csMy.questionTitle}</a></div>
