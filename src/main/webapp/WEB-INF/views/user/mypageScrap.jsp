...
function fn_mySearch()
{
	...
	document.myForm.action = "/user/mypageScrap";
...
function fn_boardMyScrapDelete(idk)
{
	
	if(confirm("스크랩을 삭제 하시겠습니까?") == true)
	{
		$.ajax({
			type:"POST",
			url:"/user/myScrapDelete",
			data:{
				commuIdk:idk
			},
			datatype:"JSON",
			...
			success:function(response)
			{
				...
				{
					alert("게시물이 삭제 되었습니다.");
					location.href = "/user/mypageScrap";
				}
...
function deleteValue() {
    ...
    if (checkValue == "") {
        ...
        if (chk) { // 사용자가 "확인"을 클릭한 경우에만 실행
            $.ajax({
                type: "POST",
                url: "/user/myScrapSelectDelete",
                data: {
                    valueArr: checkValue
                },
                datatype: "JSON",
                ...
                success: function (jdata) {
                    if (jdata.code === 0) {
                        alert("삭제 성공");
                        location.replace("/user/mypageScrap");
                    } else {
                        alert("삭제 실패");
                    }
                }
    ...
    <c:forEach var="commuMy" items="${list}" varStatus="status">	
        <c:choose>
            <c:when test="${commuMy.commuTab eq 1}">
                <c:set var="categoryName" value="마이룸" />
            </c:when>
            <c:when test="${commuMy.commuTab eq 2}">
                <c:set var="categoryName" value="자유게시판" />
            </c:when>
            <c:when test="${commuMy.commuTab eq 3}">
                <c:set var="categoryName" value="질문게시판" />
            </c:when>
            <c:otherwise>
        ...
        <div class="content-box">
            ...
            <div class="button"><button class="btnDelete" onClick="fn_boardMyScrapDelete(${commuMy.commuIdk})"><i class="fa-solid fa-trash-can"></i></button></div>			   
        </div>
    </c:forEach>
</c:if>
			   </div>
