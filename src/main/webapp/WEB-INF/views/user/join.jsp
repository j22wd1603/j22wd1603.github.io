...
$(document).ready(function() {
   ...
   $("#btnReg").on("click", function() {
	...
	$("#mailCheckInput").on("input", function() {
		    // 숫자 이외의 문자를 제거하고 숫자만 남기기
		    $("#mailCheckInput").val($("#mailCheckInput").val().replace(/[^0-9]/g, ""));
	});
	...
	$("#mailCheckInput").blur(function () {
		...
		if(inputCode === code){
			...
			$("#mailCheckMsg").html("인증번호가 불일치 합니다. 다시 확인해주세요!.");
...
function mailTimer() {
    ...
    if(maillTime > 0) {
      ...
      setTimeout(mailTimer, 1000);
		...
		$("#userEmail1").attr("disabled",false);
		$("#mailCheckBtn").attr("disabled",false);
		$("#mailCheckInput").attr("disabled",true);
		$("#mailCheckInput").val("");
		$("#countdown").text(""); //글만지움
		$("#mailCheckMsg").html(""); // 같이 설정한 css와 글을 지움
		alert("다시 인증해주세요.");
    }
  }
